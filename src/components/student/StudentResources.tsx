      ...
      <CardContent className="pt-0">
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="animate-pulse bg-muted rounded-md p-4 h-32"></div>
            ))}
          </div>
        ) : error ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {displayedResources.map((resource) => (
              <Card key={resource._id} className="overflow-hidden border hover:shadow-md transition-shadow">
                <div className="p-4">
